...
In this post, we will analyze a [random DCRat sample](https://bazaar.abuse.ch/sample/cfe65a88ebc858c083c6bfd48d1caf16128a420d9352b46c3107b8b1a1614639/) from [MalwareBazaar](https://bazaar.abuse.ch/browse/tag/arm/) using ILSpy and pythonnet. We will also implement Sigma, Suricata and YARA rules to detect the malware and a Python script to extract the encrypted configuration.
...
```
{% include_file external/threat-detection-rules/dcrat/dcrat.yar %}
```
...
```yml
{% include_file external/threat-detection-rules/dcrat/dcrat-remote-access-trojan.yml %}
```
...
```
{% include_file external/threat-detection-rules/dcrat/dcrat.rules %}
```
...
```py
{% include_file external/reversing-scripts/dcrat_config_extractor.py %}
```